...
def dynamically_modify_train_config(config: DictConfig):
    with open_dict(config):
        ...
        if mdl_name == 'rnndet':
            ...
            if backbone_name == 'MaxViTRNN':
            ...
            elif backbone_name in ('SNNCNN', 'SNNCNNRecurrent', 'PlateauSNNCNN', 'SNNCNNLSTM'):
                # SNN-CNN needs resolution aligned to total stride (32) for clean spatial division.
                ...
                print(f'Set {backbone_name} backbone (height, width) to {mdl_hw}')
                backbone_cfg.in_res_hw = mdl_hw
            elif backbone_name == 'SoftmaxSwin':
                # Same stride structure as SNNSwin.
                mdl_hw = _get_modified_hw_multiple_of(hw=dataset_hw, multiple_of=32)
                print(f'Set {backbone_name} backbone (height, width) to {mdl_hw}')
                backbone_cfg.in_res_hw = mdl_hw
            else:
